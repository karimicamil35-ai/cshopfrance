...
export async function onRequestGet({ request, env }) {
  ...
  if (incoming.searchParams.get('state') !== readCookie(request, 'cshop_oauth_state')) {
    return new Response('Connexion refusée : vérification de sécurité invalide.', { status: 400 });
  }
  ...
  const body = new URLSearchParams({
    ...
    redirect_uri: `${origin(request)}/api/auth/google/callback`,
  ...
  const value = await makeSession(
    { email: profile.email, name: profile.name || profile.email },
    env,
  );

  const next = readCookie(request, 'cshop_oauth_next');
  const destination = ['suivi', 'avis', 'administration', 'demande'].includes(next)
    ? `/?page=${next}`
    : '/';
