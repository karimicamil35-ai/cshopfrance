...
$('#requestForm').onsubmit = async event => {
  ...
  const request = Object.fromEntries(new FormData(form)); delete request.photos;
  try { const result = await api('/api/orders', { method: 'POST', body: JSON.stringify(request) }); $('#formMessage').textContent = `Demande n°${result.id} envoyée. Elle est maintenant visible dans « Mes commandes ».`; form.reset(); window.setTimeout(showOrders, 350); } catch (error) { $('#formMessage').textContent = error.message; }
};
