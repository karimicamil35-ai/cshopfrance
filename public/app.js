...
function orderCard(order) {
  ...
  return `<article class="order"><p class="status">${escapeHtml(order.status_label)}</p><b>${escapeHtml(order.product)}</b><p>${escapeHtml(order.size || 'Sans taille')} · Budget : ${escapeHtml(order.budget || 'non indiqué')}</p><p class="orderDate">Envoyée le ${date}</p><button class="chatToggle" data-chat="${order.id}">Discuter avec C SHOP.FR</button><div class="chat hidden" id="chat-${order.id}"></div>${review}</article>`;
}
...
function adminCard(order) { const states = [['new','Demande reçue'],['product_to_pay','Produit à payer'],['product_paid','Produit payé'],['shipping_to_pay','Livraison à payer'],['shipped','Colis expédié'],['receipt_confirmation','Réception à confirmer'],['closed','Terminée']]; return `<article class="order adminOrder"><div class="adminSummary"><p class="status">${escapeHtml(order.status_label)}</p><b>#${order.id} · ${escapeHtml(order.product)}</b><p><strong>${escapeHtml(order.first_name)} ${escapeHtml(order.last_name)}</strong> · ${escapeHtml(order.customer_email)} · ${escapeHtml(order.phone)}</p><p>${escapeHtml(order.city)} · ${escapeHtml(order.size || 'Sans taille')} · Budget : ${escapeHtml(order.budget || 'non indiqué')}</p><p>${escapeHtml(order.details || 'Aucun détail complémentaire')}</p></div><div class="adminControls"><label>STATUT <select data-status="${order.id}">${states.map(([value, text]) => `<option value="${value}" ${value === order.status ? 'selected' : ''}>${text}</option>`).join('')}</select></label><button class="updateButton" data-update="${order.id}">Mettre à jour</button></div><div class="adminChat"><button class="chatToggle" data-chat="${order.id}">Discuter avec le client</button><div class="chat hidden" id="chat-${order.id}"></div></div></article>`; }
